...
mod splash {
    ...
    pub fn splash_plugin(app: &mut App) {
        app.add_systems(OnEnter(GameState::Splash), splash_setup).add_systems(
            Update,
            countdown.run_if(in_state(GameState::Splash))
        );
    }
...
mod game {
    use bevy::{ color::palettes::basic::{ BLUE, LIME }, prelude::* };

    use super::{ DisplayQuality, GameState, Volume, TEXT_COLOR };

    pub fn game_plugin(app: &mut App) {
        app.add_systems(OnEnter(GameState::Game), game_setup).add_systems(
            Update,
